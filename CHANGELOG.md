...
- **Sidecars are read 32 at a time** — recomputing archived days from raw makes a trends request read thousands of tiny sidecars instead of one small digest, and `readSidecars` awaited each file in turn, which is almost entirely latency. Reads are now bounded-concurrent; kept sidecars still come back in filename order, so ordering, `files` and `parseErrors` are unchanged. A 14-day window over the real archive went 45.2s → 7.2s cold and 1449ms → 225ms warm, with every digest value identical. Every endpoint that reads sidecars gets the same speedup.
...
- **Tool overhead is a trend again, not a single day** — the card read `0` for every archived day and only ever plotted today. Past days are served by the finalized `digest.json` an external summary job writes, and that job's digest schema has drifted from core's: it never learned to emit `toolOverheadPctOfInput` (it still writes the older, different `toolBytesPctOfRequest`), so `normalizeDigest` zero-filled the field across the whole window. `buildTrends` already preferred a better source — recompute the day from its raw sidecars — but only looked in `<logDir>/archive/<date>/`, while the job parks them in `<archiveDir>/<date>/raw/` beside the digest, so that path never hit and the lossy digest served history even for days whose full raw capture was still on disk. `readArchivedDay` now searches both layouts, taking the first that holds each UTC folder rather than merging so a day present in both is counted once, and both roots stay optional — a device with neither archive still works off live `logs/` alone. Every day inside the archive's raw-retention window comes back in full: real tool overhead (14–35% on the sample fortnight, in place of `0`), untruncated `topTools` (12, not the archive's 8), and reporting-zone-correct request counts instead of the job's UTC bucketing — one day was recovered outright, and another climbed 2226 → 2700 requests once the evening that UTC had filed under the next day came home. Days past that window keep the finalized digest and stay at `0`; that data is genuinely gone. The external writer's schema drift is untouched here and remains the upstream fix.
