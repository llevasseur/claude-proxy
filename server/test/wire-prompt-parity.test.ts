 ...
 * API and the backfill, `proxy/system-prompt.mjs` for the proxy, which stays
 * zero-dependency and so cannot import TypeScript. This is the test that keeps
 * them honest: a drift in either shows up as a stored outline that disagrees
 * with the one the dashboard recomputes.
 */
...
// @ts-expect-error — plain .mjs with no types, which is the point of the parity check.
import { hashPrompt as proxyHash, outlineWirePrompt as proxyOutline, sectionsOfText as proxySections } from "../../proxy/system-prompt.mjs";
...
const CASES: { name: string; system: unknown }[] = [
  ...
  { name: "multi-byte headings and bodies", system: [block("# Héllo — wörld\n日本語の本文\n## Ünicode\némoji 🎉 here")] },
