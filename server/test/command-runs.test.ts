...
describe("reconcileCommandRuns", () => {
  ...
  // `/clear` runs in the CLI and never reaches the model, so the tokens of a session it
  // opened belong to whatever was asked next — not to `/clear`.
  describe("a session opened by a local command", () => {
    ...
    // The store can only append, so a record written under an earlier parse is retracted
    // by writing it back retired rather than removed.
    it("retires a record whose thread no longer reads as a run", async () => {
      ...
      expect(await readCommandRuns(logDir)).toEqual([]);
      // And it stays retired: the pass has nothing left to retract.
      expect(await reconcileCommandRuns(logDir, commandsDir, new Date("2026-07-15T18:00:00.000Z"))).toMatchObject({
        written: 0,
      });
    });
