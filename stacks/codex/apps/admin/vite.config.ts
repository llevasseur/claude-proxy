...
// The stack root, `stacks/codex/`, not the repository root. See ADR 0054.
const stackRoot = resolve(adminDir, '../..');

export default defineConfig(({ mode }) => {
  const adminEnv = loadEnv(mode, adminDir, '');
  const stackEnv = loadEnv(mode, stackRoot, '');
  // The server's own resolution order: scoped name first, legacy bare name second. See ADR 0050.
