...
test("sidecar write is atomic: temp file in same directory, then rename", async () => {
  ...
  await writeSanitizedSidecarAtomically(directory, candidate(), {
    beforeRename: async (temporaryPath) => {
      assert.ok(
        temporaryPath.startsWith(`${directory}/`),
        "temp file lives in the target directory",
      );
      ...
      await assert.rejects(
        stat(
          join(
            directory,
            "2026-08-22T12-00-00.000Z_11111111-2222-4333-8444-555555555555.audit.json",
          ),
        ),
      );
  ...
  const parsed = parseSanitizedAuditSidecar(
    JSON.parse(await readFile(join(directory, finalName), "utf8")),
  );
...
test("an invalid sidecar candidate is rejected before any file is created", async () => {
  const directory = await mkdtemp(join(tmpdir(), "ox-alpha-audit-test-"));
  const invalid = { ...candidate(), requestId: undefined } as unknown as ReturnType<
    typeof makeSidecar
  >;
