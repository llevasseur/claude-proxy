...
// Relative AUDIT_DIR, PROXY_STATUS_FILE and CAPTURE_DIR resolve from here, not
// process.cwd(); absolute values still win.
const REPOSITORY_ROOT = resolve(import.meta.dirname, "..", "..");

// 8787 is claimed by every sibling proxy on this machine.
...
export function loadProxyConfig(environment: NodeJS.ProcessEnv = process.env): ProxyConfig {
  ...
  return Object.freeze({
    ...
    statusFile: resolve(
      REPOSITORY_ROOT,
      environment.PROXY_STATUS_FILE ?? `${auditDirectory}/proxy-status.json`,
    ),
