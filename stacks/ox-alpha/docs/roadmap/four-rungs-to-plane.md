...
| Prompt-cache breakpoint injection | `proxy/cache-breakpoint.ts` | Claude-specific request rewriting; adapt or record a Responses-contract rationale. | unresolved — record an explicit rationale citing [ADR 0001](../adrs/0001-use-responses-contract.md) (OpenAI caches server-side), or implement an equivalent hook closed by a case in `proxy/test/forwarding.test.ts` |
| Skim / system-prompt request rewriting | `proxy/skim.ts`, `proxy/system-prompt.ts` | Decide whether any body-rewriting surface survives the transparent surface. | unresolved — each closes as an implemented hook with a forwarding test, or `N/A` here citing [ADR 0007](../adrs/0007-transparent-http-surface.md) |
...
| Live rolling usage at the proxy | `proxy/usage-live.ts` | Rolling per-process usage published for live display. | unresolved — extend the proxy status payload or audit path with rolling counters; close with a new case in `proxy/test/status.test.ts` visible on `/api/events` |
