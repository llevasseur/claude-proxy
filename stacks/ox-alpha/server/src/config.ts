...
export function readConfig(
  ...
  const base = Object.freeze({
    ...
    reconcileIntervalMs: integer(environment.RECONCILE_INTERVAL_MS, 5000, 'RECONCILE_INTERVAL_MS', 100),
    keepaliveIntervalMs: integer(environment.SSE_KEEPALIVE_MS, 15000, 'SSE_KEEPALIVE_MS', 100),
  });
  const captureDirectory = resolve(cwd, environment.CAPTURE_DIR ?? join(dirname(auditDirectory), 'captures'));
  return Object.freeze({
    ...
    captureRetentionMs: integer(environment.CAPTURE_RETENTION_MS, 604_800_000, 'CAPTURE_RETENTION_MS', 1),
