...
describe('Usage limits API', () => {
  test('meters configured rolling windows and omits unconfigured ones', async () => {
    ...
    const temporary = await temporaryDirectory();
    cleanups.push(temporary.cleanup);
    const service = new LiveUsageService(config(temporary.path, { usageLimitCeilings: { '5h': 1000 } }), () => now);
    services.push(service);
    await writeSidecar(temporary.path, 'metered.audit.json', sidecar('metered', '2026-08-19T17:00:00.000Z'));
    await writeSidecar(temporary.path, 'old.audit.json', sidecar('old', '2026-08-19T10:00:00.000Z'));
    const address = await service.start();
    ...
    const empty = (await fetch(`http://${other.host}:${other.port}/api/limits`).then((r) => r.json())) as {
      windows: unknown[];
    };
