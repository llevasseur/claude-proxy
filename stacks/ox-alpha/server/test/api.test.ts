...
describe("Bike API", () => {
  ...
  test("serves complete and explicitly unavailable aggregate costs", async () => {
    ...
    expect(
      await fetch(`${complete.origin}/api/summary`).then((response) => response.json()),
    ).toMatchObject({
      requestCount: 1,
      inputTokens: 10,
      outputTokens: 4,
    ...
    const unavailable = await start(now, async (directory) => {
      await writeSidecar(
        directory,
        "unknown.audit.json",
        sidecar("unknown", undefined, { unavailable: true }),
      );
    });
    expect(
      await fetch(`${unavailable.origin}/api/summary`).then((response) => response.json()),
    ).toMatchObject({
      requestCount: 1,
      cost: null,
  ...
  test("uses report-timezone boundaries across midnight and daylight-saving changes", async () => {
    ...
    const { origin } = await start(now, async (directory) => {
      await writeSidecar(
        directory,
        "before.audit.json",
        sidecar("before", "2026-03-08T04:59:59.000Z"),
      );
      await writeSidecar(
        directory,
        "today.audit.json",
        sidecar("today", "2026-03-08T05:00:00.000Z"),
      );
    });
    const summary = await fetch(`${origin}/api/summary`).then((response) => response.json());
    expect(summary).toMatchObject({
