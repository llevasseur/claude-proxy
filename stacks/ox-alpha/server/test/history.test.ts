...
const fetchJson = async <T>(url: string): Promise<T> =>
  (await fetch(url).then((response) => response.json())) as T;
...
describe("history and trends API", () => {
  ...
  test("buckets trends by report-timezone calendar dates across a DST boundary", async () => {
    ...
    const trends = await fetchJson<TrendsResponse>(
      `${origin}/api/trends?from=2026-03-07&to=2026-03-09`,
    );
  ...
  test("applies exact multi-select model filters to both endpoints", async () => {
    ...
    const { origin } = await start(now, async (dir) => {
      ...
      await writeSidecar(
        dir,
        "b.audit.json",
        sidecar("b", "2026-08-18T12:00:00.000Z", { model: "claude-x" }),
      );
    ...
    const twoModels = await fetchJson<HistoryResponse>(
      `${origin}/api/history?model=gpt-5&model=claude-x`,
    );
  ...
  test("advances the SSE data-version monotonically as ingest changes history", async () => {
    ...
    await writeSidecar(
      temporary.path,
      "backfill.audit.json",
      sidecar("backfill", "2026-08-01T12:00:00.000Z"),
    );
    await service.reconcile();
  ...
  test("serves identical history and trends after a full database rebuild", async () => {
    ...
    const first = await start(now, async (dir) => {
      ...
      await writeSidecar(
        dir,
        "two.audit.json",
        sidecar("two", "2026-08-01T12:00:00.000Z", { unavailable: true }),
      );
    });
    const historyBefore = await fetchJson<PaginatedHistoryRecords & HistoryResponse>(
      `${first.origin}/api/history`,
    );
