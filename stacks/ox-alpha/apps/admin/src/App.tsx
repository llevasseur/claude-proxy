  ...
  return (
    <>
      <nav className='primary-nav' aria-label='Primary'>
        {NAV.map((entry) => (
          <a key={entry.name} href={entry.href} aria-current={route.name === entry.name ? 'page' : undefined}>
            {entry.label}
          </a>
        ))}
      </nav>
      {route.name === 'history' ? (
        ...
        <TrendDetailPage date={route.search.get('date') ?? undefined} filters={validateCarSearch(route.search)} />
      ) : route.name === 'boat' ? (
        <BoatContextPage
...
function HistoryRouteView({ search }: { readonly search: ReturnType<typeof validateHistorySearch> }) {
  return (
    <HistoryPage
      filters={search}
      page={search.page ?? 1}
