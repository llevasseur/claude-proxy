...
function mergeOptions(modelOptions: readonly string[], selected: readonly string[]): readonly string[] {
...
export function FilterBar({ filters, modelOptions, onChange }: FilterBarProps) {
  ...
  const apply = (patch: { from?: string | null; to?: string | null; models?: readonly string[] }) => {
  ...
  const toggleModel = (model: string) => {
    apply({
      models: models.includes(model) ? models.filter((entry) => entry !== model) : [...models, model],
    });
  };
    ...
    <section className='card car-filters' aria-label='Range and model filters'>
      ...
      <details className='car-multiselect'>
        ...
        <div className='car-multiselect-list'>
          {options.length === 0 && (
            <p className='muted'>No models observed yet. They appear here as requests are recorded.</p>
          )}
          {options.map((model) => (
