...
describe('History route', () => {
  it('paginates forward and back through the offset contract', async () => {
    ...
    await waitFor(() =>
      expect(urls(fetchMock).some((url) => url.includes('limit=25') && url.includes('offset=25'))).toBe(true),
    );
    await waitFor(() =>
      expect(within(screen.getByTestId('history-table')).getAllByText('page-two-model').length).toBeGreaterThan(0),
    );
  ...
  it('disables Next on the last page and resets to page one when the page size grows', async () => {
    const fetchMock = stubFetch((url) => {
      ...
      return historyPageResponse([historyRecord({ recordId: `r-${params.get('limit')}-${params.get('offset')}` })], {
        total: 30,
        offset: Number(params.get('offset')),
        limit: Number(params.get('limit')),
        nextOffset: Number(params.get('limit')) === 25 && Number(params.get('offset')) === 0 ? 25 : null,
        dataVersion: 1,
      });
    });
    ...
    await waitFor(() =>
      expect(urls(fetchMock).some((url) => url.includes('limit=50') && url.includes('offset=0'))).toBe(true),
    );
  });
  ...
  it('wires the model multi-select to the exact-match parameter with empty meaning all', async () => {
    ...
    renderShell();
    await waitFor(() => expect(within(screen.getByTestId('history-table')).getByText('gpt-5').tagName).toBe('TD'));
    ...
    await waitFor(() => expect(urls(fetchMock).some((url) => url.includes('model=gpt-5'))).toBe(true));
  ...
  it('sends range filters and clears them with the filter bar', async () => {
    ...
    await waitFor(() => expect(window.location.hash).toBe('#/history?from=2026-08-01&to=2026-08-19'));
    expect(urls(fetchMock).some((url) => url.includes('from=2026-08-01') && url.includes('to=2026-08-19'))).toBe(true);
  ...
  it('refetches when an SSE data-version signal advances past the rendered payload', async () => {
    ...
    renderShell();
    await waitFor(() =>
      expect(within(screen.getByTestId('history-table')).getByText('version-one-model')).toBeTruthy(),
    );
    ...
    expect(FakeEventSource.instances.filter((entry) => entry.url === '/api/events')).toHaveLength(1);
