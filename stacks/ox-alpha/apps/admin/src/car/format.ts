...
export function unavailableReasonText(reason: CostUnavailableReason | null): string {
  return reason === null ? 'cost cannot be computed for every request' : describeUnavailableCost(reason);
}
...
export function formatTimestamp(timestamp: string): string {
  ...
  return new Intl.DateTimeFormat('en-US', { dateStyle: 'short', timeStyle: 'medium' }).format(new Date(ms));
}
