...
export function TrendDetailPage({
  ...
  return (
    <section className="car-page" aria-labelledby={titleId}>
      <Breadcrumbs
        crumbs={[{ label: "Trends", href: "#/trends" }, { label: date ?? "Latest day" }]}
      />
      <header className="pagehead">
        <div className="pagehead-title">
          <h1 id={titleId}>{date ? `Trend detail · ${date}` : "Trend detail"}</h1>
          <div className="muted">
            {data ? `${data.total} requests on this day` : "One day's captured usage"}
          </div>
        </div>
                ...
                {data.records.map((record) => {
                  ...
                  return (
                    <tr key={record.recordId}>
                      ...
                      <td className={cost.unavailable ? "cost-unavailable-text" : "car-cost"}>
                        {cost.text}
                      </td>
                    </tr>
                  );
