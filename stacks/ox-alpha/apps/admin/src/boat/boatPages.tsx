...
function PageShell({
  ...
  return (
    <section className='car-page' aria-labelledby={titleId}>
      ...
      {error && (
        <div className='card notice notice--error' role='alert' data-testid={`${testIdPrefix}-error`}>
...
function PaginatedTable<T>({
  ...
  return (
    <nav className='car-pagination' aria-label='Inspection pages'>
      <button type='button' onClick={() => onPageChange(Math.max(0, page.offset - limit))} disabled={page.offset === 0}>
        Previous
      </button>
      <span className='muted'>
        Showing {page.total === 0 ? 0 : page.offset + 1}–{page.offset + page.records.length} of {page.total}
      </span>
      <button
        type='button'
...
function BoatListPage<T>({ title, subtitle, testIdPrefix, emptyText, query, children }: BoatPageProps<T>) {
  ...
  return (
    <PageShell
      ...
      error={query.isError}
      empty={!query.isPending && !query.isError && query.data !== undefined && query.data.records.length === 0}
      emptyText={emptyText}>
    ...
    <BoatListPage
      ...
      {(records) => (
        <div className='card car-table-card'>
              ...
              {records.map((record) => (
                <tr key={record.recordId}>
                  ...
                  <td className='car-cell-mono'>
                    <a href={`#/boat/messages?recordId=${encodeURIComponent(record.recordId)}`}>{record.recordId}</a>
                    {' · '}
                    <a href={`#/boat/prompt?recordId=${encodeURIComponent(record.recordId)}`}>prompt</a>
                  </td>
          ...
          {query.data && <PaginatedTable page={query.data} onPageChange={(next) => onSearchChange({ offset: next })} />}
        </div>
      )}
    </BoatListPage>
  );
          ...
          <table className='car-table'>
            ...
            <tbody>
              {sections.data.sections.map((section) => (
                <tr key={section.kind === 'instructions' ? 'instructions' : `message-${section.index}`}>
                  <td>{section.kind === 'instructions' ? 'instructions' : `input #${section.index}`}</td>
                  <td>{section.role ?? '—'}</td>
                  <td>{section.chars}</td>
                </tr>
              ))}
            ...
            <tbody>
              {records.map((group) => (
                <tr key={group.sessionId}>
                  <td className='car-cell-mono'>
                    <a href={`#/boat/sessions/detail?id=${encodeURIComponent(group.sessionId)}`}>{group.sessionId}</a>
                  </td>
                ...
                <tr key={cohort.key}>
                  <td className='car-cell-mono'>
                    {cohort.identified && cohort.hash ? (
                      <a href={`#/boat/prompts?hash=${encodeURIComponent(cohort.hash)}`}>{cohort.label}</a>
                    ) : (
                      cohort.label
                    )}
                  </td>
            ...
            <tbody>
              {records.map((entry) => (
                <tr key={entry.recordId}>
                  ...
                  <td className='car-cell-mono'>
                    <a href={`#/boat/prompt?recordId=${encodeURIComponent(entry.recordId)}`}>{entry.recordId}</a>
                  </td>
                  <td>{entry.model ?? '—'}</td>
...
export function BoatSessionDetailPage({ id }: { readonly id?: string }) {
  ...
  return (
    <section className='car-page' aria-labelledby={titleId}>
      ...
      {detail.isError && (
        <div className='card notice notice--error' role='alert' data-testid='boat-session-detail-error'>
      ...
      {breakdown.data && breakdown.data.captures > 0 && (
        <dl className='card' data-testid='boat-session-breakdown'>
          ...
          <dt>Models</dt>
          <dd>{breakdown.data.models.map((entry) => `${entry.model} × ${entry.requests}`).join(', ') || '—'}</dd>
          <dt>Hours</dt>
          <dd>{breakdown.data.hours.map((entry) => `${entry.hour} × ${entry.captures}`).join(', ') || '—'}</dd>
        </dl>
      )}
      ...
      {detail.data && detail.data.records.length === 0 && !capture.loading && capture.enabled !== false && (
