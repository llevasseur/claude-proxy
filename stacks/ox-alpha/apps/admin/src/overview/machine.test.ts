...
describe("computeConnectionStatus", () => {
  it("starts in bootstrapping until a snapshot arrives", () => {
    expect(computeConnectionStatus({ ...LIVE, hasSnapshot: false, lastSignalAgeMs: null })).toBe(
      "bootstrapping",
    );
  });
  ...
  it("goes stale once connected but silent past the keepalive budget", () => {
    expect(computeConnectionStatus({ ...LIVE, lastSignalAgeMs: STALE_THRESHOLD_MS + 1 })).toBe(
      "stale",
    );
  });
