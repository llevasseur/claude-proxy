...
describe('capture envelope v1', () => {
  ...
  test('rejects unsupported schema versions', () => {
    expect(() => parseCaptureEnvelope(envelope({ schemaVersion: 2 }))).toThrow(/schemaVersion is unsupported/);
  });

  test('rejects non-UTC timestamps and non-path endpoints', () => {
    expect(() => parseCaptureEnvelope(envelope({ capturedAt: '2026-08-22 12:00:00' }))).toThrow(/capturedAt/);
    expect(() => parseCaptureEnvelope(envelope({ endpoint: 'v1/responses' }))).toThrow(/endpoint must start with/);
  });
});
