...
import {
  ...
  SANITIZED_AUDIT_SIDECAR_SCHEMA_VERSION,
} from "../src/sidecar.ts";
...
describe("parseSanitizedAuditSidecar", () => {
  ...
  it("rejects unknown top-level fields", () => {
    expect(() => parseSanitizedAuditSidecar({ ...valid, prompt: "leak" })).toThrow(
  ...
  it("rejects unknown nested fields in usage and cost", () => {
    ...
    expect(() =>
      parseSanitizedAuditSidecar({ ...valid, cost: { ...validCost, currencyExtra: 1 } }),
    ).toThrow(/unknown field/);
  });
  ...
  it("rejects unsupported schema versions", () => {
    expect(() => parseSanitizedAuditSidecar({ ...valid, schemaVersion: 2 })).toThrow(
  ...
  it("validates the timestamp as ISO UTC only", () => {
    expect(() =>
      parseSanitizedAuditSidecar({ ...valid, timestamp: "2026-03-08T01:30:00-04:00" }),
    ).toThrow(/timestamp/);
    expect(() => parseSanitizedAuditSidecar({ ...valid, timestamp: "not-a-time" })).toThrow(
      /timestamp/,
    );
  });

  it("requires endpoint to be a pathname without query or body slot", () => {
    expect(() => parseSanitizedAuditSidecar({ ...valid, endpoint: "v1/responses" })).toThrow(
      /endpoint/,
    );
    expect(parseSanitizedAuditSidecar({ ...valid, endpoint: "/v1/responses?x=1" }).endpoint).toBe(
      "/v1/responses?x=1",
    );
  });

  it("bounds responseStatus to valid HTTP codes", () => {
    expect(() => parseSanitizedAuditSidecar({ ...valid, responseStatus: 99 })).toThrow(
      /responseStatus/,
    );
    expect(() => parseSanitizedAuditSidecar({ ...valid, responseStatus: 600 })).toThrow(
      /responseStatus/,
    );
  });
  ...
  it("enforces detail-subset and total invariants on stored usage", () => {
    ...
    expect(() =>
      parseSanitizedAuditSidecar({ ...valid, usage: { ...validUsage, totalTokens: 999 } }),
    ).toThrow(/totalTokens/);
  });
