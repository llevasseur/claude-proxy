...
describe('resolveCalendarRange', () => {
  ...
  it('handles the spring-forward day as a 23-hour window', () => {
    ...
    expect(range.endExclusive.getTime() - (range.startInclusive?.getTime() ?? 0)).toBe(23 * 60 * 60 * 1000);
  });
  ...
  it('handles the fall-back day as a 25-hour window', () => {
    ...
    expect(range.endExclusive.getTime() - (range.startInclusive?.getTime() ?? 0)).toBe(25 * 60 * 60 * 1000);
  });
  ...
  it('rejects an inverted range and invalid inputs', () => {
    expect(() => resolveCalendarRange('2026-03-10', '2026-03-09', new Date())).toThrow(/must precede/);
    expect(() => resolveCalendarRange('2026-02-30', null, new Date())).toThrow(/invalid calendar date/);
...
describe('aggregateDailyBuckets', () => {
  ...
  it('includes events at the half-open boundaries exactly once', () => {
    ...
    const buckets = aggregateDailyBuckets(events, '2026-03-09', '2026-03-10', now, 'America/New_York');
  ...
  it('produces exact DST spring-forward bucket windows', () => {
    ...
    const buckets = aggregateDailyBuckets(events, '2026-03-08', '2026-03-09', now, 'America/New_York');
  ...
  it('produces exact DST fall-back bucket windows', () => {
    ...
    const buckets = aggregateDailyBuckets(events, '2026-11-01', '2026-11-02', now, 'America/New_York');
  ...
  it('supports an open-ended range anchored on the earliest event', () => {
    ...
    expect(buckets.map((bucket) => bucket.date)).toEqual(['2026-03-07', '2026-03-08', '2026-03-09', '2026-03-10']);
    expect(buckets[0]?.requestCount).toBe(1);
  ...
  it('rejects a sidecar timestamp that is not a valid instant', () => {
    expect(() =>
      aggregateDailyBuckets([sidecar({ timestamp: 'not-a-time' })], '2026-03-09', '2026-03-10', now),
...
describe('aggregateRangeFromBuckets', () => {
  ...
  it('makes bucket sums equal range totals through one shared path', () => {
    ...
    const buckets = aggregateDailyBuckets(events, '2026-03-09', '2026-03-11', now, 'America/New_York');
    ...
    expect(summary.cachedInputTokens).toBe(buckets.reduce((sum, bucket) => sum + bucket.cachedInputTokens, 0));
    expect(summary.outputTokens).toBe(buckets.reduce((sum, bucket) => sum + bucket.outputTokens, 0));
    expect(summary.reasoningOutputTokens).toBe(buckets.reduce((sum, bucket) => sum + bucket.reasoningOutputTokens, 0));
...
describe('model filters', () => {
  ...
  it('supports multi-select via multiple exact values', () => {
    expect(selectByModels(records, ['gpt-5', 'gpt-5-nano']).map((record) => record.id)).toEqual([1, 3]);
  });
...
describe('history record projection', () => {
  it('projects sanitized per-record values for a listing without request data', () => {
    ...
    expect(projected).toEqual(projectHistoryRecords([sidecar({ requestId: 'req_123', recordId: event.recordId })]));
  });
