...
const TOP_LEVEL_KEYS = ['schemaVersion', 'recordId', 'capturedAt', 'endpoint', 'requestText', 'responseText'] as const;
...
function exactKeys(value: Record<string, unknown>): void {
  ...
  if (unknown.length > 0) throw new CaptureValidationError(`capture envelope contains unknown field ${unknown[0]}`);
  if (missing.length > 0) throw new CaptureValidationError(`capture envelope is missing field ${missing[0]}`);
}
