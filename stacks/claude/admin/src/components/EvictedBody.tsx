    ...
    <>
      <div className='card'>
        ...
        <p className='muted'>
          ...
          {data.day ? ` (archived ${data.day})` : ''}. Its audit sidecar is kept permanently, so the measurements below
          are complete — only the request text itself is gone.
        </p>
      </div>
      ...
      {r ? (
        <>
          <div className='grid stats'>
            <StatTile
              label='Total request'
              value={fmtBytes(r.request.totalBytes)}
              sub={`~${fmtInt(Math.round(r.request.totalBytes / 4))} tokens`}
            />
