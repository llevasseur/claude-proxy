...
describe('summarizePromptMix', () => {
  ...
  it('groups by captured hash and reports contributions summing to the mean', () => {
    const day = summarizePromptMix(
      [...cohort(3, 'opus', 17_000, 'aaaa1111'), ...cohort(1, 'opus', 111_000, 'bbbb2222')],
      '2026-08-02',
    );
  ...
  it("skips malformed entries, so the mean matches the digest's own population", () => {
    const day = summarizePromptMix(
      [...cohort(2, 'opus', 17_000), { timestamp: '2026-08-02T14:00:00.000Z' }, null],
      '2026-08-02',
    );
  ...
  it('separates two hashes that happen to be the same size', () => {
    const day = summarizePromptMix(
      [...cohort(1, 'opus', 17_000, 'aaaa'), ...cohort(1, 'opus', 17_000, 'bbbb')],
      '2026-08-02',
    );
...
describe('attributePromptMix', () => {
  it('charges a pure traffic shift to mix and nothing to size', () => {
    const prior = summarizePromptMix(
      [...cohort(8, 'opus', 17_000, 'a'), ...cohort(2, 'opus', 111_000, 'b')],
      '2026-08-01',
    );
    const current = summarizePromptMix(
      [...cohort(5, 'opus', 17_000, 'a'), ...cohort(5, 'opus', 111_000, 'b')],
      '2026-08-02',
    );
  ...
  it('always splits the whole move, with both effects at once', () => {
    ...
    const current = summarizePromptMix(
      [...cohort(3, 'opus', 19_000, 'a'), ...cohort(6, 'opus', 105_000, 'b'), ...cohort(1, 'opus', 4_000, 'c')],
      '2026-08-02',
    );
  ...
  it('treats a cohort seen on only one day as pure mix', () => {
    const prior = summarizePromptMix(cohort(4, 'opus', 17_000, 'a'), '2026-08-01');
    const current = summarizePromptMix(
      [...cohort(3, 'opus', 17_000, 'a'), ...cohort(1, 'opus', 111_000, 'b')],
      '2026-08-02',
    );
  ...
  it('reports a null percentage when the prior day had no bytes', () => {
    const at = attributePromptMix(
      summarizePromptMix([], '2026-08-01'),
      summarizePromptMix(cohort(1, 'opus', 17_000, 'a'), '2026-08-02'),
    );
...
describe('pairPromptRevisions', () => {
  ...
  it('ignores a prompt that survived the day', () => {
    const prior = summarizePromptMix(cohort(2, 'opus', 17_000, 'aaaa1111'), '2026-08-01');
    const current = summarizePromptMix(
      [...cohort(2, 'opus', 17_000, 'aaaa1111'), ...cohort(1, 'opus', 21_000, 'bbbb2222')],
      '2026-08-02',
    );
    expect(pairPromptRevisions(prior, current)).toEqual([]);
  });
  ...
  it('orders several revisions by the size of the move', () => {
    const prior = summarizePromptMix(
      [...cohort(2, 'opus', 17_000, 'a1'), ...cohort(2, 'sonnet', 9_000, 'b1')],
      '2026-08-01',
    );
    const current = summarizePromptMix(
      [...cohort(2, 'opus', 18_000, 'a2'), ...cohort(2, 'sonnet', 40_000, 'b2')],
      '2026-08-02',
    );
