...
describe("parseCommandSteps", () => {
  ...
  it("stops a step's body at an unrelated heading", () => {
    const steps = parseCommandSteps(["## Step 1 — A", "run `pnpm install`", "## Notes", "run `gh pr create`"].join("\n"));
    expect(steps[0]?.artifacts).toEqual([{ kind: "shell", value: "pnpm install" }]);
  });

  it("merges a step whose heading repeats, rather than declaring it twice", () => {
    const steps = parseCommandSteps(
      ["## Step 1 — A", "`pnpm install`", "## Step 1 — A again", "`npm ci`"].join("\n"),
    );
...
describe("parseCommandEnvelope", () => {
  ...
  // Verbatim shape of what the CLI sends when a local command opens a turn: the caveat,
  // the local envelope, its empty stdout, then whatever the user actually typed.
  ...
  it("reads each envelope's own args, not the first block in the prompt", () => {
    // `/clear` carries empty args; charging them to the command after it emptied its prompt.
    expect(parseCommandEnvelope(`${local("clear")} ${envelope("fb", "fix the dialog")}`)?.prompt).toBe(
      "fix the dialog",
    );
  });

  // The caveat's text survives into a compaction summary, far from any envelope it says
  // nothing about. Only an envelope the caveat directly precedes is a local command.
...
describe("attributeSteps", () => {
  ...
  it("fills forward from an anchor until the next one", () => {
    ...
    expect(attributions.map((a) => a.confidence)).toEqual([
      "explicit",
      "inferred",
      "inferred",
      "explicit",
  ...
  const withArtifacts: CommandStep[] = [
    ...
    { id: "3", order: 3, title: "Clean, then PR", artifacts: [
      { kind: "skill", value: "clean" },
      { kind: "skill", value: "pr" },
    ] },
  ];
...
describe("detectPatterns", () => {
  ...
  it("fires when a step's first act is an error", () => {
    const nodes = [decision(0, "Step 2 — shipping"), node(1, "error", "Bash(command=gh pr create)", "Bash(command=gh pr create)")];
    const fired = detectPatterns(nodes, attributeSteps(nodes, steps));
...
describe("stepReach", () => {
  ...
  it("keeps unreached steps in the funnel and appends the unattributed bucket", () => {
    const stats = (reached: boolean) => [
      { step: "1", title: "", reached: true, confidence: null, tokens: ZERO_TOKENS, cost: 0, turns: 0, nodes: 0, toolCalls: 0, waste: { erroredTools: 0, duplicateReads: 0, retriedAfterError: 0, noOpTurns: 0, cacheMissTokens: 0 } },
      { step: "2", title: "", reached, confidence: null, tokens: ZERO_TOKENS, cost: 0, turns: 0, nodes: 0, toolCalls: 0, waste: { erroredTools: 0, duplicateReads: 0, retriedAfterError: 0, noOpTurns: 0, cacheMissTokens: 0 } },
    ];
...
describe("filterRunsByFlags", () => {
  const runs = [run({ threadId: "a", flags: ["sub"] }), run({ threadId: "b", flags: [] }), run({ threadId: "c", flags: ["sub", "draft"] })];
