...
describe('deriveSessionName', () => {
  ...
  it('names a compacted session for the continuation, not its predecessor summary', () => {
    const prompt =
      'This session is being continued from a previous conversation that ran out of context. ' +
      'The summary below covers the earlier portion of the conversation. Summary: ' +
      '## 1. Primary Request and Intent One user asked for …';
    expect(deriveSessionName(prompt)).toBe(CONTINUATION_NAME);

    // An interrupted run's marker rides in front of the preamble.
    expect(deriveSessionName(`[Request interrupted by user] ${prompt}`)).toBe(CONTINUATION_NAME);
  });
});
...
describe('truncateWords', () => {
  ...
  it('does not leave dangling punctuation on the cut', () => {
    expect(truncateWords('resolve the flag, then update every caller', 18)).toBe('resolve the flag…');
  });
});

describe('deriveSessionPreview', () => {
  it("strips a slash command's envelope down to the arguments the name doesn't carry", () => {
    const prompt =
      '<command-message>ideate</command-message> <command-name>/ideate</command-name> ' +
      '<command-args>UI/UX improvements to the design</command-args> ' +
      'Propose what is worth building. This command surveys the repo …';
    expect(deriveSessionPreview(prompt)).toBe('UI/UX improvements to the design');
  });

  it('has no preview for a command with no arguments — the name is the whole prompt', () => {
    const bare =
      '<command-message>mc</command-message> <command-name>/mc</command-name> # mc — Merge main & resolve conflicts';
    expect(deriveSessionPreview(bare)).toBeNull();
  });
  ...
  it('drops the caveat the CLI prepends to a locally-run command', () => {
    const prompt =
      '<local-command-caveat>Caveat: The messages below were generated by the user while running local commands.</local-command-caveat>' +
      '<command-name>/clear</command-name> <command-message>clear</command-message>';
    expect(deriveSessionPreview(prompt)).toBeNull();
  });

  it('says a compacted session was continued instead of quoting the summary', () => {
    const prompt =
      'This session is being continued from a previous conversation that ran out of context. ' +
      'The summary below covers the earlier portion of the conversation. Summary: ' +
      '1. **Primary Request and Intent:** the user asked for …';
    expect(deriveSessionPreview(prompt)).toBe(CONTINUATION_PREVIEW);
  });

  it('keeps a plain prose prompt as typed, collapsed to one line', () => {
    expect(deriveSessionPreview('Add Monitor back to the default\nclaude space (do not deny it)')).toBe(
      'Add Monitor back to the default claude space (do not deny it)',
    );
  });

  it('strips injected context, and cuts a long prompt on a word boundary', () => {
    expect(deriveSessionPreview('<system-reminder>\nbe good\n</system-reminder>\n\nship the fix')).toBe('ship the fix');

    const long = deriveSessionPreview(`ship the fix ${'word '.repeat(80)}`)!;
    expect(long.endsWith('…')).toBe(true);
    expect(long.length).toBeLessThanOrEqual(181);
    // The cut landed between words, so nothing before the `…` is a fragment.
    expect(long.slice(0, -1).endsWith('word')).toBe(true);
  });

  it('has nothing to preview from an empty or reminder-only prompt', () => {
    expect(deriveSessionPreview(null)).toBeNull();
    expect(deriveSessionPreview('   ')).toBeNull();
    expect(deriveSessionPreview('<system-reminder> context that never closes')).toBeNull();
  });
});
...
describe('sessionName / sessionDisplayName', () => {
  ...
  describe('sessionPreview', () => {
    it('carries the rest of a name that was cut short', () => {
      const m = meta({
        derivedTitle: '/mc -t fix/mechanical-closi…',
        subtitle:
          '<command-message>mc</command-message> <command-name>/mc</command-name> ' +
          '<command-args>-t fix/mechanical-closing-turn-and-merge-gates</command-args> Merge main …',
      });
      expect(sessionPreview(m)).toBe('-t fix/mechanical-closing-turn-and-merge-gates');
    });

    it('says nothing when the name above it already does', () => {
      // The name was not cut, so it carries the arguments whole and the line would repeat them.
      const m = meta({
        derivedTitle: '/improve --ideas',
        subtitle:
          '<command-message>improve</command-message> <command-name>/improve</command-name> ' +
          '<command-args>--ideas</command-args> Turn session suggestions into an improvement …',
      });
      expect(sessionPreview(m)).toBeNull();

      // A continued session's name and preview are the same sentence at two lengths.
      const continued = meta({
        derivedTitle: CONTINUATION_NAME,
        subtitle: 'This session is being continued from a previous conversation that ran out of context. Summary: …',
      });
      expect(sessionPreview(continued)).toBeNull();
    });

    it("keeps a preview the CLI's own title says nothing about", () => {
      const m = meta({ title: 'Merge gates', subtitle: 'fix the mechanical closing turn and merge gates' });
      expect(sessionPreview(m)).toBe('fix the mechanical closing turn and merge gates');
    });

    it('has no preview when the prompt has none to give', () => {
      expect(sessionPreview(meta({}))).toBeNull();
      expect(
        sessionPreview(
          meta({
            derivedTitle: '/mc',
            subtitle: '<command-message>mc</command-message> <command-name>/mc</command-name> # mc',
          }),
        ),
      ).toBeNull();
    });
