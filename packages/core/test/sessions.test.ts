...
describe("parseSessionTranscript", () => {
  ...
  it("derives a name from the opening prompt, whether or not the CLI titled it", () => {
    expect(parseSessionTranscript("ab3167129339d34f", TRANSCRIPT).derivedTitle).toBe(
...
describe("deriveSessionName", () => {
  ...
  it("names a slash command by the command and its arguments", () => {
    ...
    const bare = "<command-message>mc</command-message> <command-name>/mc</command-name> # mc — Merge main & resolve conflicts";
  ...
  it("drops the caveat the CLI prepends to a locally-run command", () => {
    const prompt =
      ...
      "<command-message>clean</command-message> <command-name>/clean</command-name>";
    expect(deriveSessionName(prompt)).toBe("/clean");
  });
