...
describe("suggestBucket", () => {
  it("flags guardrail refusals as high severity and names the sessions they hit", () => {
    const sessions = [
      session("a1", day(1), ["## Task: One", "- Bash(command=rm -rf /)", "- ✗ Permission denied by hook", "- done: ok"]),
  ...
  it("reports an error that recurs across sessions once, with its occurrence count", () => {
    const sessions = [
      session("b1", day(1), ["## Task: One", "- Bash(command=pnpm test)", "- ✗ ENOENT: no such file /a.ts", "- done: ok"]),
      session("b2", day(2), ["## Task: Two", "- Bash(command=pnpm test)", "- ✗ ENOENT: no such file /b.ts", "- done: ok"]),
    ];
