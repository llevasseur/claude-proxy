...
describe('skimDigestsByDay', () => {
  it('splits by reporting-zone day, oldest first', () => {
    const d1 = makeSidecar({
      timestamp: '2026-07-14T10:00:00.000Z',
      skim: makeSkim({ enabled: true, servedFromCache: true, savedInputTokens: 100, cacheKey: 'a' }),
    });
    const d2 = makeSidecar({
      timestamp: '2026-07-15T10:00:00.000Z',
      skim: makeSkim({ enabled: true, servedFromCache: false, cacheKey: 'b' }),
    });
  ...
  it('keeps a late-evening request on the day it was made', () => {
    ...
    const evening = makeSidecar({
      timestamp: '2026-07-16T01:30:00.000Z',
      skim: makeSkim({ enabled: true, servedFromCache: true, savedInputTokens: 100, cacheKey: 'a' }),
    });
    const morning = makeSidecar({
