...
`/v1/messages` requests from a local response cache — on a hit it returns a stored
reply and makes **zero call to Anthropic** — and instruments every request so hit-rate
and dollars saved are measurable. The skim defaults **off** and never changes the bytes
Claude Code sees on a miss (the proxy stays a transparent pass-through). Reaching the
destination means a rough prototype that short-circuits real traffic, plus the audit
data needed to study whether the skim is worth keeping. A dashboard page to *study* that
data is a named future phase, not the destination itself.
...
- **Domain:** the claude-proxy monorepo. `proxy/proxy.mjs` is a **zero-dependency**
  ...
  is the pure/tested analysis lib, `server/` the HTTP API over `logs/` — answering
  from the SQLite substrate by default, with a narrow set of write routes
  (`WRITE_ROUTES` in `server/src/server.ts`) — and `apps/admin` the dashboard.
...
- **Two cache layers, don't conflate them** (established with the user before charting):
  Anthropic's *prefix cache* is server-side
  transformer KV-state, ~90% off input tokens, 5-min/1-hr TTL, exact-prefix + per-org —
  **cannot** be moved to the proxy without self-hosting the model. The skim is an
  *app-layer response cache*: caches model **output**; a hit saves the **entire** API
  call and works cross-session / cross-user. This map builds the second, not the first.
...
- [Skim dashboard to study results](tickets/005-skim-dashboard.md) — core now
  aggregates the sidecar `skim` fields (hit/miss, hit-rate, saved tokens, est. $
  saved via `pricing.ts`); `server/` exposes `GET /api/skim` + `/api/skim/trend`;
  `apps/admin` gains a "Skim" route charting hit-rate over time, cumulative $
  saved, and top repeated request shapes.
...
- **Semantic skim layer.** Embed the request and vector-match "same or similar task"
  above a threshold, instead of byte-exact hashing. High payoff (real agent traffic is
  rarely byte-identical) but carries wrong-answer risk and needs a threshold + scope
  keys. Can't be sharply ticketed until the exact-match prototype shows the real
  byte-exact hit-rate floor — graduates once ticket 001's data is in.
- **Cost-model / routing skim.** Routing cheap asks to a smaller model instead of
  caching. Adjacent lever, same seat in the proxy; revisit after the cache data lands.
