...
Measured in claude-proxy alone: **104** occurrences of `--filter @agent-proxy/claude-server` and 26 of
`--filter @agent-proxy/concepts`. Only 155 of the 184 rename sites across the three repos are
...
invokes `pnpm --filter @agent-proxy/claude-server maintain --apply` with `WorkingDirectory` set to the repo,
...
4. **`deploy-concepts.yml` needs both halves.** Blocker (f) covers the `paths` trigger and
   not the `pnpm --filter @agent-proxy/concepts` argument, so the fix as written repairs the trigger
...
6. **The sweep is a judgement ticket, not a mechanical one.** `--filter @agent-proxy/claude-server` names one
